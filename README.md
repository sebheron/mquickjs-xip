...
I was having problems with reading bytecode and executing it directly from the ROM with Mquickjs for platforms that differ from where the bytecode was originally written.

One of the solutions I saw in some forks was to use the repl. This can be quite intensive on limited platforms.
Also the default bytecode path requires relocation on the target platform itself, which on a read-only platform means loading the bytecode into RAM - probably impossible if you're really constrained, but also defeats the point of running from ROM in the first place.
